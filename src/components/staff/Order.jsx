...
const Order = () => {
        ...
        const fetchItems = async () => {
            try {
                ...
                const itemsWithCategoryData = response.data.map(item => ({
                    ...
                    CategoryLocation: categories.find(cat => cat.Name === selectedCategory)?.Location || 'No Kitchen',
                }));

                setItems(itemsWithCategoryData.length === 0 ? [] : itemsWithCategoryData);
        ...
        setCart((prevCart) =>
            prevCart.map((cartItem) => {
                ...
                if (isSameItem) {
                    return {
                        ...cartItem,
                        quantity: type === 'increase' ? cartItem.quantity + 1 : Math.max(1, cartItem.quantity - 1),
                    };
                }
    ...
    const calculateItemPrice = (item) => {
        ...
        const addOnsPrice = item.addOns ? item.addOns.reduce((acc, addOn) => acc + addOn.Price, 0) : 0;
        return basePrice + addOnsPrice;
    };

    const calculateTotalPrice = () => {
        return cart.reduce(
            (total, cartItem) => total + calculateItemPrice(cartItem) * cartItem.quantity,
            0
        );
    };
    ...
    const handleNoteChange=(value)=>{
      ...
      const handlePaymentMethodChange = async (method) => {
        try {
            ...
            let status = method === 'Proceed' ? 'Pending' :
                method === 'Pay Later' ? 'Pay Later' : 'Done';

            const payload = {
                Status: status,
                PaymentType: method === 'Proceed' || method === 'Pay Later' ? 'N/A' : method,
                Items: cart.map((item) => ({
                    CategoryName: item.CategoryName,
            ...
            if (isUpdate) {
                await axios.put(`${process.env.REACT_APP_API_URL}/Order/UpdateOrderByOrderNumber/${orderNumber}`, payload);
                createdOrderNumber = orderNumber;
            } else {
                const response = await axios.post(`${process.env.REACT_APP_API_URL}/Order/CreateOrder`, payload);
...
printData.push(
  ...
  },
  {
      ...
      style: { textAlign: 'center', fontSize: '16px', fontWeight: 'bold', color: 'blue' },
  },
  {
      type: 'text',
      value: `Date: ${new Date().toLocaleDateString('fr-FR')}    Time: ${new Date().toLocaleTimeString('fr-FR')}`,
  ...
  if (item.removals && item.removals.length > 0) {
      item.removals.forEach(removal => {
          ...
          printData.push({
              type: 'text',
              value: removalName,
              style: { textAlign: 'left', fontSize: '12px', color: 'red', fontStyle: 'italic' },
          });
      });
  }
  ...
  if (item.addOns && item.addOns.length > 0) {
      item.addOns.forEach(addOn => {
          ...
          printData.push({
              type: 'text',
              value: addOnName,
              style: { textAlign: 'left', fontSize: '12px', color: 'green', fontStyle: 'italic' },
          });
      });
  }
});
  ...
  },
  {
      ...
      style: { textAlign: 'center', fontSize: '14px', fontStyle: 'italic' },
            ...
            cart.forEach(item => {
                ...
                const name = item.Name.length > nameWidth ? item.Name.substring(0, nameWidth - 3) + '...' : item.Name.padEnd(nameWidth, ' ');
                const qty = item.quantity.toString().padStart(Math.floor(qtyWidth / 2) + item.quantity.toString().length % 2).padEnd(qtyWidth, ' ');
                const price = (calculateItemPrice(item) * item.quantity).toFixed(2).toString().padStart(priceWidth, ' ');
                ...
                if (item.addOns && item.addOns.length > 0) {
                    item.addOns.forEach(addOn => {
                        ...
                        const addOnPrice = (addOn.Price * item.quantity).toFixed(2).toString().padStart(priceWidth, ' ');
                        cashierPrintData.push({
                            type: 'text',
                            value: `${addOnName.padEnd(nameWidth + qtyWidth, ' ')}${addOnPrice} CFA`,
                            style: { textAlign: 'left', fontSize: '12px', fontFamily: 'monospace', color: 'green' },
                        });
                    });
                }
                ...
                if (item.removals && item.removals.length > 0) {
                    item.removals.forEach(removal => {
                        ...
                        cashierPrintData.push({
                            type: 'text',
                            value: `${removalName}`,
                            style: { textAlign: 'left', fontSize: '12px', fontFamily: 'monospace', color: 'red' },
                        });
                    });
                }
            });
            ...
            cashierPrintData.push(
                ...
                ...(payload.DeliveryCharge > 0 ? [
                    {
                        type: 'text',
                        value: `Delivery Charge: ${payload.DeliveryCharge.toFixed(2)} CFA`,
            ...
            const response = await window.electronAPI.printOrder(cashierPrintData, cashierPrintOptions);
            if (response.success) {
...
>
    ...
    >
      <ToggleButton 
        ...
        sx={{ 
          ...
          '&.Mui-selected': { backgroundColor: '#1976D2', color: '#fff' } // Blue when selected
        }}
      >
        Dine In
      </ToggleButton>
            ...
            >
              ...
              >
                {calculateItemPrice(item)} CFA
                    ...
                    {/* Cart Section */}
                    <Grid item xs={3.5} sx={{ backgroundColor: '#f0f0f0', padding: 2, boxShadow: 3, borderLeft: '1px solid #e0e0e0' }}>
            ...
            >
              <Box sx={{ flex: 1 }}>
                ...
                {cartItem.addOns && cartItem.addOns.length > 0 && (
                  ...
                  Add-ons: {cartItem.addOns.map((add) => `${add.Name} (${add.Price} CFA)`).join(', ')}
                  </Typography>
                )}
              </Box>
  ...
  <DialogContent dividers sx={{ paddingX: 3, paddingY: 2 }}>
    ...
    >
      <ToggleButton value="remove" sx={{ fontWeight: 'bold', fontSize: '1.2rem', padding: '10px 20px' }}>
        Remove Ingredients
      </ToggleButton>
      <ToggleButton value="add" sx={{ fontWeight: 'bold', fontSize: '1.2rem', padding: '10px 20px' }}>
        Add Ingredients
      </ToggleButton>
    </ToggleButtonGroup>
    ...
    {activeTab === 'remove' && (
      <Box>
        <Typography variant="h6" sx={{ mt: 2, fontWeight: 'bold', textAlign: 'center', fontSize: '1.3rem' }}>
          Remove Ingredients
        </Typography>
        <Box
          sx={{
        ...
        >
          {selectedItem?.Ingredients?.map((ingredient, index) => (
            <Chip
              ...
              sx={{ fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem', padding: '10px 15px' }}
            />
          ))}
        </Box>
      </Box>
      ...
      <Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mt: 2, mb: 2 }}>
          {ingredientsCategories.map((cat, index) => (
            <Button
              key={index}
              variant={selectedIngredientCategory === cat.categoryName ? 'contained' : 'outlined'}
              onClick={() => handleIngredientCategoryChange(cat.categoryName)}
              sx={{ fontWeight: 'bold', fontSize: '1rem', padding: '10px 20px', borderRadius: '12px' }}
            >
              {cat.categoryName}
            </Button>
          ))}
  ...
  <DialogActions sx={{ justifyContent: 'space-between', padding: 3 }}>
    <Button onClick={() => setOpenDialog(false)} color="secondary" sx={{ fontWeight: 'bold', fontSize: '1.2rem', padding: '10px 20px' }}>
      Cancel
    </Button>
    <Button onClick={handleSaveItem} variant="contained" color="primary" sx={{ fontWeight: 'bold', fontSize: '1.2rem', padding: '10px 20px' }}>
      Save
    </Button>
  </DialogActions>
            </Dialog>
